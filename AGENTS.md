...
- `src/`：扩展源码。
  ...
  - `fileTree.ts`：把提交中的文件变更转换为目录树。
  - `webview.ts`：文件历史 Webview 的 HTML、样式和前端交互。
  - `i18n.ts`：运行时文案国际化。
  - `types.ts`：共享 TypeScript 类型。
- `tests/`：Node 内置 test runner 测试，覆盖 Git 解析、真实 Git 仓库历史读取、文件树和 Webview 回归。
