...
- 查看某个文件在指定提交中的改动，或将历史版本、当前文件与最新版、分支、标签、远程分支、任意 Git ref 对比；diff 会保留原始文件路径与类型，便于 VS Code 渲染历史版本代码。
...
- `src/`：扩展源码，包含命令入口、Git 历史与 Graph 查询、Git blob 虚拟文档 URI、Git Blame 状态栏、文件历史/提交详情/Git Graph Webview 渲染、i18n 和共享类型。
- `tests/`：自动化测试，覆盖 Git 解析、真实 Git 仓库查询、Git blob URI、Git Graph 数据与 Webview、Git Blame 链接解析、提交详情渲染、变更文件排序和 Webview 回归。
